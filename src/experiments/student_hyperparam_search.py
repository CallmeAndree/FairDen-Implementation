# Student Performance Hyperparameter Optimization
...
def student_hyperparameter_search(dataname: str, output_dir: str):
    """
    Hyperparameter optimization for Student Performance dataset.
    Find best minPts and epsilon based on DCSI score.
    
    Parameters:
    -----------
    dataname: str
        Name of config (e.g., 'student', 'student_address')
    output_dir: str
    ...
    minpts_candidates = sorted(list(set(minpts_candidates)))  # Remove duplicates
    ...
    with open(f'{output_dir}/{dataname}_best_params.txt', 'w') as f:
        f.write(f"Student Performance ({dataname}) - Best Hyperparameters (by DCSI)\n")
        f.write("="*50 + "\n")
...
def main():
    """Run hyperparameter search for all student configurations."""
    output_dir = 'results/student_hyperparam'
    
    configs = [
        ('student', 'sex'),
        ('student_address', 'address'),
        ('student2', 'address + sex')
    ]
    ...
    for dataname, desc in configs:
        ...
        best_params, df = student_hyperparameter_search(dataname, output_dir)
    ...
    # Final summary
    print("\n\n" + "="*80)
    print("FINAL SUMMARY: Optimal Parameters for Student Performance Dataset")
    print("="*80)
    print(f"{'Config':<20} {'Sensitive':<12} {'minPts':<8} {'eps':<8} {'DCSI':<10} {'Balance':<10} {'Clusters':<10}")
    print("-" * 90)
    
    for dataname, desc in configs:
        best = all_best[dataname]
        if best:
            print(f"{dataname:<20} {desc:<12} {best['minPts']:<8} {best['epsilon']:<8} {best['DCSI']:<10.4f} {best['Balance']:<10.4f} {best['n_clusters']:<10}")
