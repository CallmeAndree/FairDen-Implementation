# Census Hyperparameter Optimization
...
def census_hyperparameter_search(dataname: str, output_dir: str):
    """
    Hyperparameter optimization for Census dataset.
    ...
    with open(f'{output_dir}/{dataname}_best_params.txt', 'w') as f:
        f.write(f"Census ({dataname}) - Best Hyperparameters (by DCSI)\n")
        f.write("="*50 + "\n")
...
def main():
    """Run hyperparameter search for all Census configurations."""
    output_dir = 'results/census_hyperparam'
    
    # All cens configs: 1-attr, 2-attr, 3-attr (intersectional)
    configs = [
        ...
        # 3 sensitive attributes (intersectional)
    ...
    for dataname, desc in configs:
        ...
        best_params, df = census_hyperparameter_search(dataname, output_dir)
    ...
    # Final summary
    print("\n\n" + "="*80)
    print("FINAL SUMMARY: Optimal Parameters for Census Dataset")
    print("="*80)
    print(f"{'Config':<25} {'Sensitive':<20} {'minPts':<8} {'eps':<8} {'DCSI':<10} {'Balance':<10} {'Clusters':<10}")
    print("-" * 100)
    
    for dataname, desc in configs:
        best = all_best[dataname]
        if best:
            print(f"{dataname:<25} {desc:<20} {best['minPts']:<8} {best['epsilon']:<8} {best['DCSI']:<10.4f} {best['Balance']:<10.4f} {best['n_clusters']:<10}")
        else:
            print(f"{dataname:<25} {desc:<20} {'N/A':<8} {'N/A':<8} {'N/A':<10} {'N/A':<10} {'N/A':<10}")
